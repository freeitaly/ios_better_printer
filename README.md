...
**原因**：
1. **平台风控限制**：企业微信自建应用虽然在客户端能发送文件，但如果应用未配置**可信域名**，服务器端不会推送 file 类型回调。
2. **可信域名门槛**：配置可信域名需要**企业认证**（需营业执照）且域名必须经过**ICP备案**。
3. **动态域名无效**：测试证实，仅使用动态域名（DDNS）绕过IP限制是不够的，因无法完成归属权验证和备案，仍被平台拦截。

### 阶段三：放弃

**决定**：暂停项目开发。

**结论**：本项目要跑通的**硬性前提**是拥有一个**经过认证的企业微信账号**和一个**已备案的域名**。个人开发者及未认证企业无法使用此功能。

**考虑的替代方案**：
- **Web上传页面**：搭建简单的文件上传网页，完全绕开IM平台限制（可行，推荐）
- **Telegram Bot**：完全支持文件收发（可行）
- **邮件服务**：监听邮箱附件（可行）
